...
class GaussianDiffusionTrainer(nn.Module):
    def __init__(self, model: nn.Module, beta: Tuple[int, int], T: int):
        ...
        # calculate the cumulative product of alpha, named alpha_t_bar in paper
        alpha_t = 1.0 - self.beta_t
        alpha_t_bar = torch.cumprod(alpha_t, dim=0)

        # calculate and store two coefficients of q(x_t | x_0)
...
class GaussianDiffusionTrainer1(nn.Module):
    def __init__(self, model: nn.Module, beta: Tuple[int, int], T: int):
        super().__init__()
        self.model = model
        self.T = T
    ...
    def forward(self, x_0):
        ...
        # 修改的部分开始
        # 计算损失时增加权重因子，为接近1的值分配更高的权重
        weight = self.calculate_weight(x_0)
        loss = F.mse_loss(epsilon_theta, epsilon, reduction="none") * weight
        loss = torch.sum(loss)
        # 修改的部分结束

        return loss

    def calculate_weight(self, x_0):
        """
        计算基于原始像素值的权重，为接近1的像素分配更高的权重。
        """
        # 将像素值从[-1, 1]转换回[0, 255]
        x_0_unnormalized = (x_0 + 1) / 2 * 255
        # 计算权重，这里我们简单地将接近255的值（即原始值为255的像素）的权重设置为更高
        # 例如，可以使用一个简单的阈值方法或者基于像素值的函数来动态调整权重
        weight = torch.where(x_0_unnormalized > 191, 5.0 * torch.ones_like(x_0), torch.ones_like(x_0))
        return weight

class GaussianDiffusionTrainer2(nn.Module):
    def __init__(self, model: nn.Module, beta: Tuple[int, int], T: int):
        super().__init__()
        self.model = model
        self.T = T
    ...
    def calculate_weight(self, x_0):
        ...
        # 计算权重，这里我们简单地将接近255的值（即原始值为255的像素）的权重设置为更高
        # 例如，可以使用一个简单的阈值方法或者基于像素值的函数来动态调整权重
        weight = torch.where(x_0_unnormalized < 85, 1.0 * torch.ones_like(x_0),
                             torch.where((x_0_unnormalized >= 85) & (x_0_unnormalized <= 170),
                                         1.0 * torch.ones_like(x_0),
                                         torch.ones_like(x_0)))
        return weight
